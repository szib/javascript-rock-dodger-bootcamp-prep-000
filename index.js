...
function checkCollision(rock) {
  ...
  if (top > 360) {
    ...
    const dodgerRightEdge = positionToInteger(DODGER.style.left) + DODGER_WIDTH;
  
    const rockLeftEdge = positionToInteger(rock.style.left)
  
    // FIXME: The rock is 20 pixel's wide -- how do we get the right edge?
    const rockRightEdge = positionToInteger(rock.style.left) + ROCK_WIDTH;
      ...
      if (collisionCheck1 || collisionCheck2 || collisionCheck3 || (rockLeftEdge >= dodgerLeftEdge && rockRightEdge <= dodgerRightEdge)) {
          return true
      } else {
        return false
      }
